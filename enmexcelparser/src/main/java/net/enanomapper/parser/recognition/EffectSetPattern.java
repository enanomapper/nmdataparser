...
public class EffectSetPattern {
	...
	public static EffectSetPattern extractEffectSetPattern(JsonNode node, String jsonSection,
		...
		if (!sectionNode.path("SKIP_SPACES").isMissingNode()) {
			...
			if (b == null) {
				conf.addError(
						"In JSON section \"" + jsonSection + "\", keyword \"SKIP_SPACES\" : " + jsonUtils.getError());
			} else {
				esp.skipSpaces = b;
		...
		if (!sectionNode.path("EFFECT_SEPARATOR").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError("In JSON section \"" + jsonSection + "\", keyword \"EFFECT_SEPARATOR\" : "
						+ jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("INTERNAL_SEPARATOR").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError("In JSON section \"" + jsonSection + "\", keyword \"INTERNAL_SEPARATOR\" : "
						+ jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("ENDPOINT_AT_FIRST_POSITION").isMissingNode()) {
			...
			if (b == null) {
				conf.addError("In JSON section \"" + jsonSection + "\", keyword \"ENDPOINT_AT_FIRST_POSITION\" : "
						+ jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("ENDPOINT_PREFIX").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError("In JSON section \"" + jsonSection + "\", keyword \"ENDPOINT_PREFIX\" : "
						+ jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("ENDPOINT_SUFFIX").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError("In JSON section \"" + jsonSection + "\", keyword \"ENDPOINT_SUFFIX\" : "
						+ jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("VALUE_PREFIX").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError(
						"In JSON section \"" + jsonSection + "\", keyword \"VALUE_PREFIX\" : " + jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("VALUE_SUFFIX").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError(
						"In JSON section \"" + jsonSection + "\", keyword \"VALUE_SUFFIX\" : " + jsonUtils.getError());
			} else {
		...
		if (!sectionNode.path("UNIT").isMissingNode()) {
			...
			if (keyword == null) {
				conf.addError("In JSON section \"" + jsonSection + "\", keyword \"UNIT\" : " + jsonUtils.getError());
			} else {
				esp.FlagUnit = true;
				esp.unit = keyword;
			}
