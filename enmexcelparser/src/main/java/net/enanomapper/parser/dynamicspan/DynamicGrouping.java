	...
	public static DynamicGrouping extractDynamicGrouping(JsonNode node, ExcelParserConfigurator conf,
		...
		if (node.path("GROUPING_ELEMENT_INDEX").isMissingNode()) {
			conf.addError("In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\", "
					+ " subsection GROUP_LEVEL [" + (groupNum + 1)
					+ "], keyword \"GROUPING_ELEMENT_INDEX\" is missing!");
		} else {
			...
			if (col_index == -1) {
				conf.addError("In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\", "
						+ " subsection GROUP_LEVEL [" + (groupNum + 1)
						+ "], keyword \"GROUPING_ELEMENT_INDEX\" is incorrect!");
			} else {
		...
		if (node.path("GROUP_CUMULATIVE_TYPE").isMissingNode()) {
			conf.addError("In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\", "
					+ " subsection GROUP_LEVEL [" + (groupNum + 1)
					+ "], keyword \"GROUP_CUMULATIVE_TYPE\" is missing!");
		} else {
			String keyword = jsonUtils.extractStringKeyword(node, "GROUP_CUMULATIVE_TYPE", false);
			if (keyword == null)
				conf.addError("In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\", "
						+ " subsection GROUP_LEVEL [" + (groupNum + 1) + "], keyword \"GROUP_CUMULATIVE_TYPE\" :"
						+ jsonUtils.getError());
			else {
				...
				if (dyngrp.groupCumulativeType == ObjectType.UNDEFINED)
					conf.addError("In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\", "
							+ " subsection GROUP_LEVEL [" + (groupNum + 1)
							+ "], keyword \"GROUP_CUMULATIVE_TYPE\" is incorrect or UNDEFINED!  -->" + keyword);
				else
		...
		if (!node.path("ROW_TYPE").isMissingNode()) {
			...
			if (keyword == null)
				conf.addError("In JSON Section \"" + masterSection
						+ "\" subsection \"DYNAMIC_ITERATION_SPAN\", " + " subsection GROUP_LEVEL [" + (groupNum + 1)
						+ "], keyword \"ROW_TYPE\" :" + jsonUtils.getError());
			else {
				dyngrp.rowType = ObjectType.fromString(keyword);
				if (dyngrp.rowType == ObjectType.UNDEFINED)
					conf.addError("In JSON Section \"" + masterSection
							+ "\" subsection \"DYNAMIC_ITERATION_SPAN\", " + " subsection GROUP_LEVEL ["
							+ (groupNum + 1) + "], keyword \"ROW_TYPE\" is incorrect or UNDEFINED!  -->" + keyword);
				else
