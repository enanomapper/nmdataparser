...
public class DynamicElement {
	...
	public int resultObjectIndex = 0; // If id != 0 it can be used to put field
										// in a newly created Object i
	...
	public String parameterName = null; // This field is used by fieldTypes:
										// PARAMETER, CONDITION and PROPERTY

	public boolean infoFromHeader = true;
	public boolean FlagInfoFromHeader = false;

	public String variableKeys[] = null; // The information is taken
											// (constructed) form the variables
											// defined by their keys

	public int childElements[] = null; // The information is taken from another
										// element

	// public String sourceDIS = null; //The information is taken from another
	// DIS
	...
	public static DynamicElement extractDynamicElement(JsonNode node, ExcelParserConfigurator conf,
		...
		/*
		 * //DATA_TYPE if(node.path("DATA_TYPE").isMissingNode()) { if
		 * (node.path("FIELD_TYPE").isMissingNode())
		 * conf.configErrors.add("In JSON Section \"" + masterSection +
		 * "\" subsection \"DYNAMIC_ITERATION_SPAN\", " +" subsection ELEMENT ["
		 * +(elNum +1) +
		 * "], keyword + \"DATA_TYPE\" and \"FIELD_TYPE\" are missing!" +
		 * " At least one must be specified."); } else { String keyword =
		 * jsonUtils.extractStringKeyword(node, "DATA_TYPE", false); if (keyword
		 * == null) conf.configErrors.add("In JSON Section \"" + masterSection +
		 * "\" subsection \"DYNAMIC_ITERATION_SPAN\", " +" subsection ELEMENT ["
		 * +(elNum +1) + "], keyword \"DATA_TYPE\": " + jsonUtils.getError());
		 * else { element.dataType = ObjectType.fromString(keyword); if
		 * (element.dataType == ObjectType.UNDEFINED)
		 * conf.configErrors.add("In JSON Section \"" + masterSection +
		 * "\" subsection \"DYNAMIC_ITERATION_SPAN\", " +" subsection ELEMENT ["
		 * +(elNum +1) +
		 * "], keyword \"DATA_TYPE\" is incorrect or UNDEFINED!  -->" +
		 * keyword); else element.FlagDataType = true; } }
		 */
		...
		if (!node.path("FIELD_TYPE").isMissingNode()) {
			...
			if (keyword == null)
				...
				if (element.fieldType == ElementField.UNDEFINED)
					...
					if ((element.fieldType == ElementField.PARAMETER) || (element.fieldType == ElementField.CONDITION)
					...
					/*
					 * // Setting dataType / Checking field compatibility with
					 * dataType if (element.dataType == null) element.dataType =
					 * element.fieldType.getElement(); else { if
					 * (element.dataType != element.fieldType.getElement())
					 * conf.configErrors.add("In JSON Section \"" +
					 * masterSection +
					 * "\" subsection \"DYNAMIC_ITERATION_SPAN\", "
					 * +" subsection ELEMENT [" +(elNum +1) + "], FIELD_TYPE \""
					 * + element.fieldType.toString() +
					 * "\" is incompatible with DATA_TYPE \"" +
		...
		if (node.path("INDEX").isMissingNode()) {
			...
			// conf.configErrors.add("In JSON Section \"" + masterSection + "\"
			// subsection \"DYNAMIC_ITERATION_SPAN\", "
			// +" subsection ELEMENT [" +(elNum +1) + "], keyword \"INDEX\" is
			// missing!");
		} else {
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (chNode.isInt()) {
						...
						if (intVal <= 0)
							...
							element.childElements[i] = intVal - 1; // 1-based
																	// -->
																	// 0-based
					} else {
						conf.configErrors.add("In JSON Section \"" + masterSection
	...
	public String toJSONKeyWord(String offset) {
		...
		/*
		 * if (FlagDataType) { if (nFields > 0) sb.append(",\n");
	...
	public void putElementInUniversalObject(Object elObj, UniversalObject uniObj, ExcelParserConfigurator config) {
		ObjectType otype = fieldType.getObjectType();
		switch (otype) {
		case SUBSTANCE:
			SubstanceRecord sr = uniObj.getSubstanceRecord();
			putElementInSubstanceRecord(elObj, sr, config);
			break;
	...
	/*
	 * public void putElementInSubstanceRecordArray(Object elObj,
	 * ArrayList<SubstanceRecord> substanceRecords) { //TODO }
	 */

	public void putElementInSubstanceRecord(Object elObj, SubstanceRecord substanceRecord,
			ExcelParserConfigurator config) {
		if (elObj == null)
			return;
		...
		case COMPANY_UUID:
			...
			substanceRecord.setSubstanceUUID(config.generateUUID(s));
			break;
	...
	public void putElementInProtocolApplication(Object elObj, ProtocolApplication protocolApplication) {
		...
		switch (fieldType) {
		case CITATION_TITLE:
			protocolApplication.setReference(elObj.toString()); // title is the
																// reference
																// 'itself'
			break;
		case CITATION_YEAR:
	...
	public void putElementInEffectRecord(Object elObj, EffectRecord effect) {
			...
			if (elObj instanceof String) {
				...
				if (qv.value != null) {
					...
					if (qv.qualifier != null)
						effect.setLoQualifier(qv.qualifier); // this qualifier
																// takes
																// precedence
																// (if already
																// set)
		...
		case UP_VALUE:
			if (elObj instanceof String) {
				...
				if (qv.value != null) {
					...
					if (qv.qualifier != null)
						effect.setUpQualifier(qv.qualifier); // this qualifier
																// takes
																// precedence
																// (if already
																// set)
		...
		case ERR_VALUE:
			if (elObj instanceof String) {
				...
				if (qv.value != null) {
					...
					if (qv.qualifier != null)
						effect.setErrQualifier(qv.qualifier); // this qualifier
																// takes
																// precedence
																// (if already
																// set)
		...
		case VALUE:
			if (elObj instanceof String) {
				...
				effect.setLoValue((Double) elObj); // This is the default
													// behavior if the cell is
													// of type numeric
			}
			break;
	...
	public void putElementInComposition(Object elObj, CompositionRelation composition) {
		...
		// System.out.println("----> putElementInComposition: " +
		// elObj.toString());
