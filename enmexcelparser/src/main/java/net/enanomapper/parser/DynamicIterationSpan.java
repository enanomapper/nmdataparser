...
/**
 * 
 * @author nick This class defines information for dynamic extraction of
 *         information during iteration This info is typically used by the
 *         iteration modes: ROW_MULTI_*, COLUMN_MULTI_*
 * 
 * 
 */
public class DynamicIterationSpan {
	...
	public boolean handleByRows = true; // The flag is related to the iteration
										// mode and it determines whether basic
										// data elements are rows or columns
	public boolean FlagHandleByRows = false;

	public ObjectType cumulativeObjectType = null; // This is what type of
													// object is formed by the
													// cumulative effect of all
													// of rows/columns
	public ObjectType rowType = null; // This is the default row level grouping
	// public DataElementType columnType = null; //This is the default column
	// level grouping
	...
	public static DynamicIterationSpan extractDynamicIterationSpan(JsonNode node, ExcelParserConfigurator conf,
		...
		if (!node.path("PARALLEL_TO_PRIMARY").isMissingNode()) {
			...
			if (b == null)
				conf.addError("In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\" keyword "
		...
		if (node.path("CUMULATIVE_OBJECT_TYPE").isMissingNode()) {
			...
			if (keyword == null)
				...
				if (dis.cumulativeObjectType == ObjectType.UNDEFINED)
					conf.addError(
							"In JSON Section \"" + masterSection + "\" subsection \"DYNAMIC_ITERATION_SPAN\" keyword "
		...
		if (!node.path("GROUP_LEVELS").isMissingNode()) {
			...
			if (elNode.isArray()) {
				...
				conf.addError("In JSON Section \"" + masterSection
						+ "\" subsection \"DYNAMIC_ITERATION_SPAN\" keyword " + "\"GROUP_LEVELS\" is not an array!");
		}
	...
	protected boolean checkElementConsistency() {
		...
		if (rowType != null)
			for (int i = 0; i < elements.size(); i++) {
				 ...
				 * addError(masterErrorString + " ELEMENTS[" + (i+1) + "] type "
				 * + elements.get(i).dataType.toString() +
	...
	protected boolean checkGroupLevelConsistency() {
		...
		if (!groupLevels.get(0).groupCumulativeType.isElementOf(cumulativeObjectType))
			addError(masterErrorString + " GROUP_LEVELS[1].groupCumulativeType "
					+ groupLevels.get(0).groupCumulativeType.toString() + " is not an element of cumulativeObjectType "
					+ cumulativeObjectType.toString());
		...
		for (int i = 0; i < groupLevels.size(); i++) {
			...
			if (i > 0)
				if (!groupLevels.get(i).groupCumulativeType.isElementOf(groupLevels.get(i - 1).groupCumulativeType))
					addError(masterErrorString + " GROUP_LEVELS[" + (i + 1) + "].groupCumulativeType "
							+ groupLevels.get(i).groupCumulativeType.toString() + " is not an element of GROUP_LEVELS["
							+ i + "].groupCumulativeType " + groupLevels.get(i - 1).groupCumulativeType.toString());
		}
	...
	protected DynamicIterationObject handleGroupsLavels(ArrayList<Row> rows) {
		...
		TreeMap<Integer, String> groups = ExcelUtils.getRowGroups(rows, groupLevels.get(0).groupingElementIndex,
				FlagNextNonEmpty);
	...
	protected Object getElementObject(Row row, DynamicElement element, Object elementObjects[]) {
		if (element.childElements != null)
			if (elementObjects == null)
				return null; // This is the first round and nothing is done.
								// This element has children and it will be
								// processed on the second round.
		...
		if (element.FlagIndex) // Getting value from a position defined by the
								// index
		{
			Cell c = null;
			...
			case NON_FIRST_ROW: // data is loaded for any row expect the first
								// row
				if (row != firstRow)
					c = row.getCell(element.index);
				// else
			...
			case NON_FIRST_GROUP_ROW: // data is loaded for any row expect the
										// first group row
				if (row != firstGroupRow)
					c = row.getCell(element.index);
				// else
		...
		// (3) VARIABLE_KEYS
		// (4) ChildrenElements
		// If more than one source present, the information is concatenated in
